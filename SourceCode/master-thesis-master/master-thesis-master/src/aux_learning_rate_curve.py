...
def main(exp_path):
    ...
    num_epochs = exp_params["training_slots"]["num_epochs"]

    # loading data. We only need to know the number of batches per epoch
    batch_size = exp_params["training_slots"]["batch_size"]
    ...
    print(f"Num Iters per epoch: {num_iters}")
    ...
    warmup = exp_params["training_slots"]["lr_warmup"]
    warmup_steps = exp_params["training_slots"]["warmup_steps"]
    warmup_epochs = exp_params["training_slots"]["warmup_epochs"]
