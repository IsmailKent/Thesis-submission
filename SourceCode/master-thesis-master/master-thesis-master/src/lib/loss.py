...
def get_loss(loss_type="mse", **kwargs):
    ...
    elif loss_type in ["composite_prediction_loss"]:
        loss = CompositePredictionLoss()
    return loss
...
class CompositePredictionLoss(nn.Module):
    """ Combines MSE loss in slot space with with MSE loss in image space"""

    def __init__(self, weight_slot_space=1, weight_image_space=1):
        """ Module initializer """
        super().__init__()
        self.mse_slots = nn.MSELoss()
        self.mse_images = nn.MSELoss()
        self.weight_slot_space = weight_slot_space
        self.weight_image_space = weight_image_space

    def forward(self, **kwargs):
        """ Computing loss """
        preds, targets = kwargs.get("preds"), kwargs.get("targets")
        pred_imgs, target_imgs = kwargs.get("pred_imgs"), kwargs.get("target_imgs")
        loss = self.weight_slot_space * self.mse_slots(preds, targets) + \
               self.weight_image_space * self.mse_images(pred_imgs, target_imgs)
        return loss
