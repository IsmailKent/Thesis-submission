...
def setup_model(model_params):
    ...
    if model_name == "RecursiveInstanceEncoder":
        cur_model_params = model_params["SAVi"]
    else:
        cur_model_params = model_params[model_name]

    if(model_name == "SlotAttention"):
        model = models.SlotAttentionModel(**cur_model_params)
    elif(model_name == "SAVi"):
        model = models.SAVi(**cur_model_params)
    elif(model_name == "RecursiveInstanceEncoder"):
        model = models.RecursiveInstanceEncoder(**cur_model_params)
    else:
        raise NotImplementedError(f"Model '{model_name}' not in recognized models; {MODELS}")
...
def setup_predictor(exp_params):
    ...
    --------
    model: torch.nn.Module
        instanciated predictor given the parameters
    """
    # model params
    ...
    training_pred_params = exp_params["training_prediction"]
    ...
    if(predictor_name == "LSTM"):
        predictor = models.Predictors.LSTMPredictor(
    ...
    elif(predictor_name == "VanillaTransformer"):
        predictor = models.Predictors.VanillaTransformerPredictor(
                num_slots=cur_model_params["num_slots"],
                slot_dim=cur_model_params["slot_dim"],
                num_imgs=training_pred_params["sample_length"],
    ...
    elif(predictor_name == "OCVTransformerV1"):
        predictor = models.Predictors.OCVTransformerV1Predictor(
                num_slots=cur_model_params["num_slots"],
                slot_dim=cur_model_params["slot_dim"],
                num_imgs=training_pred_params["sample_length"],
                token_dim=cur_predictor_params["token_dim"],
    ...
    elif(predictor_name == "OCVTransformerV2"):
        predictor = models.Predictors.OCVTransformerV2Predictor(
                num_slots=cur_model_params["num_slots"],
                slot_dim=cur_model_params["slot_dim"],
                num_imgs=training_pred_params["sample_length"],
                token_dim=cur_predictor_params["token_dim"],
    ...
    elif(predictor_name == "MLPMixer"):
        num_seed = training_pred_params["num_context"]
        seq_length = num_seed if not training_pred_params["skip_first_slot"] else num_seed - 1
        predictor = models.Predictors.MLPMixer(
                seq_length=seq_length,
                num_objs=cur_model_params["num_slots"],
                slot_dim=cur_model_params["slot_dim"],
                token_dim=cur_predictor_params["token_dim"],
                hidden_dim=cur_predictor_params["hidden_dim"],
                num_layers=cur_predictor_params["num_layers"],
                dropout=cur_predictor_params["dropout"],
                object_mixing=cur_predictor_params["object_mixing"],
                temporal_mixing=cur_predictor_params["temporal_mixing"],
                se_weighting=cur_predictor_params["se_weighting"],
            )
    else:
        raise NotImplementedError(f"Predictor '{predictor_name}' not in recognized predictors: {PREDICTORS}")
