...
def visualize_attention_sequence(sequence, indices_max_attended, attention_values, savepath=None,  tag="attention_sequence", add_title=True, add_axis=False, n_cols=10,
                       size=3, font_size = 11, attention_type = "vanilla", n_channels=3, titles=None, tb_writer=None, iter=0, **kwargs, ):
    """ Visualizing a grid with several images/frames """
        ...
        if(add_title):
            if(titles is not None):
                ...
                if attention_type == "vanilla":
                    ...
                    a.set_title(f"Time:{time}\n Obj:{object}\n attn. val:{'%.3f' % round(attention_values[i], 3)}", fontsize = font_size)
                elif attention_type == "time":
                    time = indices_max_attended[i]
                    a.set_title(f"Time:{time}\n attn. val:{'%.3f' % round(attention_values[i], 3)}", fontsize = font_size)
                else:
                    object = indices_max_attended[i]
                    a.set_title(f"Obj:{object}\n attn. val:{'%.3f' % round(attention_values[i], 3)}", fontsize = font_size)
...
def visualize_qualitative_eval(context, targets, preds, savepath=None, context_titles=None,
                               target_titles=None, pred_titles=None):
    """
    ...
    if context_titles is None:
        ax[0, n_cols//2].set_title("Seed Frames")
    if target_titles is None:
        ax[1, n_cols//2].set_title("Target Frames")
    if pred_titles is None:
        ax[n_rows_pred, n_cols//2].set_title("Predicted Frames")
