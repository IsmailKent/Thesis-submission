...
from models import InstanceMaskEncoder
...
class MaskInit(nn.Module):
    """
    Slots are initialized by encoding instance segmentation masks
    """

    def __init__(self, slot_dim, num_slots, encoder_resolution):
        """ Module intializer """
        super().__init__()
        self.slot_dim = slot_dim
        self.num_slots = num_slots
        self.mask_encoder = InstanceMaskEncoder(
                slot_dim=slot_dim,
                num_slots=num_slots,
                encoder_resolution=encoder_resolution
            )
    ...
    def forward(self, batch_size, **kwargs):
        """ Encoding instance masks into slots """
        instance_masks = kwargs.get("instance_masks", None)
        if instance_masks is None:
            raise ValueError("Mask Initializer requires having 'instance_masks'...")

        # filling missing slots with 0-masks
        num_masks = instance_masks.shape[2]
        if num_masks > self.num_slots:
            raise ValueError(f"There shouldnt be more {num_masks = } than {self.num_slots = }! ")
        if num_masks < self.num_slots:
            remaining_masks = self.num_slots - num_masks
            H, W = instance_masks.shape[-2], instance_masks.shape[-1]
            padded_zeros = torch.zeros(remaining_masks, H, W)
            instance_masks = torch.cat([instance_masks, padded_zeros], dim=2)

        slots = self.mask_encoder(instance_masks)
        return slots
